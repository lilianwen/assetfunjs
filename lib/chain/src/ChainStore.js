...
class ChainStore
{
    ...
    */
    ...
    queryForSubjectPublish(pretictionEndTime){
        Apis.instance().db_api().exec("get_global_properties").then(chainGlobalProperties => {
            let vote_duration_percent = chainGlobalProperties.parameter.subject_profile.vote_duration_percent;
            Apis.instance().db_api().exec("get_dynamic_global_properties").then(chainDynamicProperties => {
                var result = {};
