...
var TransactionBuilder = function () {
    ...
    */
    ...
    TransactionBuilder.prototype.process_transaction = function process_transaction(cwallet) {
        ...
        if (_fidchainjsWs.Apis.instance().chain_id !== wallet_object.get("chain_id")) return Promise.reject("Mismatched chain_id; expecting " + wallet_object.get("chain_id") + ", but got " + _fidchainjsWs.Apis.instance().chain_id);
    ...
    TransactionBuilder.prototype.finalize = function finalize() {
        ...
        return new Promise(function (resolve, reject) {
            ...
            resolve(_fidchainjsWs.Apis.instance().db_api().exec("get_objects", [["2.1.0"]]).then(function (r) {
                head_block_time_string = r[0].time;
                if (_this2.expiration === 0) _this2.expiration = base_expiration_sec() + _fidchainjsWs.ChainConfig.expire_in_secs;
    ...
    */
    ...
    TransactionBuilder.prototype.get_type_operation = function get_type_operation(name, operation) {
        ...
        if (name === 'proposal_create') {
            ...
            */
            ...
            operation.expiration_time || (operation.expiration_time = base_expiration_sec() + _fidchainjsWs.ChainConfig.expire_in_secs_proposal);
            if (requiresReview) {
                operation.review_period_seconds = extraReview + Math.max(committee_min_review, 24 * 60 * 60 || _fidchainjsWs.ChainConfig.review_in_secs_committee);
                /*
    ...
    TransactionBuilder.prototype.update_head_block = function update_head_block() {
        return Promise.all([_fidchainjsWs.Apis.instance().db_api().exec("get_objects", [["2.0.0"]]), _fidchainjsWs.Apis.instance().db_api().exec("get_objects", [["2.1.0"]])]).then(function (res) {
            var g = res[0],
                r = res[1];
    ...
    TransactionBuilder.prototype.set_required_fees = function set_required_fees(asset_id) {
        ...
        var promises = [_fidchainjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, asset_id])];

        var feeAssetPromise = null;
        if (asset_id !== "1.3.0") {
            // This handles the fallback to paying fees in BTS if the fee pool is empty.
            promises.push(_fidchainjsWs.Apis.instance().db_api().exec("get_required_fees", [operations, "1.3.0"]));
            promises.push(_fidchainjsWs.Apis.instance().db_api().exec("get_objects", [[asset_id]]));
        }
        ...
        return Promise.all(promises).then(function (results) {
            ...
            var dynamicPromise = asset_id !== "1.3.0" && asset ? _fidchainjsWs.Apis.instance().db_api().exec("get_objects", [[asset.dynamic_asset_data_id]]) : new Promise(function (resolve, reject) {
                resolve();
            });
    ...
    TransactionBuilder.prototype.get_potential_signatures = function get_potential_signatures() {
        ...
        return Promise.all([_fidchainjsWs.Apis.instance().db_api().exec("get_potential_signatures", [tr_object]), _fidchainjsWs.Apis.instance().db_api().exec("get_potential_address_signatures", [tr_object])]).then(function (results) {
    ...
    TransactionBuilder.prototype.get_required_signatures = function get_required_signatures(available_keys) {
        ...
        return _fidchainjsWs.Apis.instance().db_api().exec("get_required_signatures", [tr_object, available_keys]).then(function (required_public_keys) {
    ...
    TransactionBuilder.prototype.sign = function sign() {
        var chain_id = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : _fidchainjsWs.Apis.instance().chain_id;
...
function _broadcast(was_broadcast_callback) {
    ...
    return new Promise(function (resolve, reject) {
        ...
        _fidchainjsWs.Apis.instance().network_api().exec("broadcast_transaction_with_callback", [function (res) {
            return resolve(res);
